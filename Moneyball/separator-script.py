...
with open('data/kraj.csv', newline='') as source:
    dialect = csv.Sniffer().sniff(source.readline())
    source.seek(0)
    reader = csv.reader(source, dialect)
    number_of_columns = len(next(reader))
    source.seek(0)
    with open("data/kraj-brani.csv", "w") as goalkeepers_file:
        gk_writer = csv.writer(goalkeepers_file)
        with open("data/svi.csv", "w") as players_file:
